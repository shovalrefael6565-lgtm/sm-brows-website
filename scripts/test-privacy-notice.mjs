...
chk('🔒 אין הצהרה על מחיקה אוטומטית מבוססת-זמן', !/נמחק אוטומטית|מנגנון מחיקה אוטומטי מבוסס-זמן פעיל/.test(privacyContentSrc))
chk('🔒 אין יותר הצהרה מטעה שנתוני OTP/session "נשמרים לפרק זמן קצר"',
  !/נשמרים לפרק זמן קצר/.test(privacyContentSrc))
chk('🔒 מצהיר במפורש שהרשומות הטכניות של אימות/הפעלות אינן נמחקות כולן אוטומטית כיום',
  /הרשומות הטכניות הקשורות לניסיונות אימות[\s\S]{0,60}אינן נמחקות כולן באופן אוטומטי מבסיס הנתונים/.test(privacyContentSrc))
chk('🔒 מציין תוקף שימוש מוגבל לקודי אימות ולהפעלות כניסה',
  /תוקף שימוש מוגבל/.test(privacyContentSrc))
chk('🔒 מפנה את קביעת תקופות השמירה/הניקוי הייעודי למדיניות שמירה עתידית (לא מבטיח מנגנון קיים)',
  /תקופות שמירה ומנגנון ניקוי ייעודי[\s\S]{0,20}ייקבעו במסגרת מדיניות השמירה/.test(privacyContentSrc))
