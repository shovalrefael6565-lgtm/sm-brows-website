 ...
 *
 * ⚠️ 🔴 **אותו דבר חל מ-15E על reschedule_appointment_by_customer עצמה:
 * היא נמחקה מהמסד ב-0023 (DROP FUNCTION) ואינה קיימת בפרודקשן.**
 *
 * הקובץ הזה ממשיך לבדוק אותה בכוונה, מאותו שיקול בדיוק — הוא טוען
 * 0001–0005 ברשימה מפורשת (לא readdirSync), ולכן 0023 לעולם אינה נכנסת
 * אליו וה-PGlite שלו עדיין מכיל את הפונקציה. זהו תיעוד חי של מה ש-0005
 * הבטיחה בנקודת הזמן שלה, ואין למחוק אותו.
 *
 * 🔒 **אין להסיק מכאן שהמסלול הזה חי.** שינוי מועד מ-15E ואילך הוא
 * *בקשה* שמצריכה אישור, ולא UPDATE מיידי. המסלול האמיתי נבדק ב-
 * scripts/test-reschedule-request-db.mjs, שם גם עברו לשם NEW_IN_PAST
 * ו-SYNC_IN_PROGRESS (סעיף 15) — שני השומרים שהיו מכוסים רק כאן.
 *
