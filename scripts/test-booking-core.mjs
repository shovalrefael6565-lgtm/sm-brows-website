...
//
// ⚠️ הטווח היה "עד 6 ימי עסקים" (≈שבוע), אחר כך 30 יום, ומ-24.09.2026 הוא
// 37 (כדי שכל אוקטובר ייפתח). ההרחבה נוגעת **אך ורק** בשאלה עד כמה רחוק
// מותר להסתכל. כל שאר החסימות — שישי/שבת, יומן Google, חגים שנחסמו ביומן,
// תורים קיימים — נבדקות במנגנון שלהן ולא השתנו.
section('טווח ההזמנה — 37 ימים קדימה')

chk('הקבוע הוא 37 ימים', BOOKING_HORIZON_DAYS === 37)
...
chk('גבול עליון: יום 37 בדיוק (10.09) בטווח', isWithinBookingHorizon(2026, 8, 10, now))
chk('🔒 יום 38 (11.09) כבר מחוץ לטווח', !isWithinBookingHorizon(2026, 8, 11, now))

// (1) יום בתוך הטווח שנפתח **רק** בזכות ההרחבה: 20.08 הוא חמישי,
//     16 ימים קלנדריים / 12 ימי עסקים מהיום, ואינו בשום חלון מיוחד.
chk('✅ (1) יום בתוך הטווח, מחוץ לחלון הישן ומחוץ לזמינות המיוחדת — פתוח',
  isBookableDate(2026, 7, 20, now))
chk('✅ (1) גם +7 ימי עסקים (13.8) פתוח עכשיו', isBookableDate(2026, 7, 13, now))

// (2) מעבר לטווח — לא מוצג. 14.09 הוא שני, 41 יום קדימה (מעבר ל-37),
//     ומחוץ לכל החלונות המיוחדים (האחרון מסתיים 10.09).
chk('🔒 (2) מעבר לטווח (14.09, 41 יום) — סגור', !isBookableDate(2026, 8, 14, now))
...
chk('🔒 (3) שבת 22.08 סגורה למרות שהיא בתוך הטווח', !isBookableDate(2026, 7, 22, now))
chk('🔒 (3) שבת 29.08 סגורה גם בתוך חלון הזמינות המיוחדת',
  !isBookableDate(2026, 7, 29, now) && isFridayOrSaturday(2026, 7, 29))
chk('🔒 (3) שישי 21.08 סגור למרות שהוא בתוך הטווח', !isBookableDate(2026, 7, 21, now))
{
  ...
  chk('🔒 (3) אף שישי/שבת בכל ימי הטווח אינו פתוח', openWeekend === 0, `נפתחו=${openWeekend}`)
}
...
// ⚠️ מאז ההרחבה ל-37 יום, 08.09 נמצא **בתוך** הטווח ביחס ל-`now` (04.08),
// ולכן האינווריאנטה נבדקת מול נקודת זמן מוקדמת יותר: מ-01.08, יום 08.09 הוא
// 38 יום קדימה — מעבר לטווח — ועדיין נמצא בחלון הערב המיוחד (23.08–09.09).
// זו אותה בדיקה בדיוק: מעבר לטווח הרשת המלאה אינה חלה, רק מה שנפתח במפורש.
{
  const nowEarly = new Date('2026-08-01T09:00:00Z')
  chk('🔒 מעבר לטווח (08.9 מ-01.08) — רק הסלוטים המיוחדים: 17:00 כן, 09:00 לא',
    !isWithinBookingHorizon(2026, 8, 8, nowEarly) &&
    isValidTimeSlot(2026, 8, 8, '17:00', nowEarly) &&
    !isValidTimeSlot(2026, 8, 8, '09:00', nowEarly))
}
...
// (12) טווח ההזמנה — 37 יום, והחשיפה ההדרגתית אינה חורגת ממנו
chk('🔒 (12) booking horizon הוא 37 יום, והחשיפה ההדרגתית אינה חורגת ממנו',
  BOOKING_HORIZON_DAYS === 37 &&
  selectDisplaySlots({ year: 2026, month: 8, day: 14, busyRanges: [], durationMin: 20, now }).length === 0)

// ════════════════════════════════════════════════════════════════════════════
section('אוקטובר 2026 פתוח במלואו — 37 יום מ-24.09.2026')
// ════════════════════════════════════════════════════════════════════════════

{
  // נקודת הזמן שבה בוצעה ההרחבה. ב-30 יום, יום 30 נפל על 24.10 ו-25–31.10
  // היו מוסתרים; 37 מביא בדיוק עד 31.10.
  const oct = new Date('2026-09-24T09:00:00Z')
  const dow = d => new Date(2026, 9, d).getDay()

  chk('יום 37 מ-24.09 הוא בדיוק 31.10', calendarDayOffset(2026, 9, 31, oct) === 37)
  chk('🔒 ב-30 יום 31.10 היה מחוץ לטווח (37 > 30) — זו בדיוק הסיבה להרחבה',
    calendarDayOffset(2026, 9, 31, oct) > 30 && BOOKING_HORIZON_DAYS >= 37)

  // כל ימי העבודה (א׳–ה׳) באוקטובר פתוחים
  const workdays = []
  for (let d = 1; d <= 31; d++) if (dow(d) !== 5 && dow(d) !== 6) workdays.push(d)
  const closedWorkdays = workdays.filter(d => !isBookableDate(2026, 9, d, oct))
  chk(`✅ כל ${workdays.length} ימי העבודה באוקטובר פתוחים להזמנה`,
    closedWorkdays.length === 0, closedWorkdays.length ? `סגורים: ${closedWorkdays.join(',')}` : '')

  // 🔒 ההרחבה לא פתחה שישי/שבת — כולל 30.10 (ו׳) ו-31.10 (ש׳)
  const weekend = []
  for (let d = 1; d <= 31; d++) if (dow(d) === 5 || dow(d) === 6) weekend.push(d)
  const openWeekend = weekend.filter(d => isBookableDate(2026, 9, d, oct))
  chk(`🔒 אף אחד מ-${weekend.length} ימי שישי/שבת באוקטובר לא נפתח`,
    openWeekend.length === 0, openWeekend.length ? `נפתחו: ${openWeekend.join(',')}` : '')
  chk('🔒 31.10 הוא שבת ולכן נשאר סגור למרות שהוא בתוך הטווח',
    isWithinBookingHorizon(2026, 9, 31, oct) && !isBookableDate(2026, 9, 31, oct))

  // 🔒 הגבול עדיין קיים: 01.11 (א׳, 38 יום) מחוץ לטווח
  chk('🔒 01.11 (38 יום) עדיין מחוץ לטווח — הטווח לא נפתח לרווחה',
    !isWithinBookingHorizon(2026, 10, 1, oct) && !isBookableDate(2026, 10, 1, oct))

  // 🔒 השעות והסולם לא זזו בגלל ההרחבה
  const midOct = { year: 2026, month: 9, day: 14, busyRanges: [], now: oct } // רביעי
  chk('🔒 יום באוקטובר משתמש באותה רשת שעות בדיוק',
    legalStartTimes({ ...midOct, durationMin: 20 }).every(t => BUSINESS_SHIFTS.some(sh =>
      mins(t) >= mins(sh.start) && mins(t) + 20 <= mins(sh.end))))
  chk('🔒 והסולם באוקטובר הוא 8 (יום רחוק), לא "כל הזמינות"',
    selectDisplaySlots({ ...midOct, durationMin: 20 }).length === INITIAL_SLOTS)

  // 🔒 חסימת יומן/תור קיים ממשיכה לחסום גם בימים החדשים שנפתחו (25–29.10)
  const blocked = selectDisplaySlots({
    year: 2026, month: 9, day: 28, durationMin: 20, now: oct,
    busyRanges: [{ start: '00:00', end: '23:59' }],
  })
  chk('🔒 יום שנחסם ביומן בתוך הימים החדשים (28.10) נשאר סגור', blocked.length === 0)
}
