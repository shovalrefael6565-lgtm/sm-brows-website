...
export function selectVisibleSlots(params: SelectSlotsParams): string[] {
  ...
  // פיזור בין שתי המשמרות. ⚠️ מאז 09.09.2026 הבוקר ארוך יותר (12 סלוטים
  // מול 9 בערב), אבל ההטיה לערב וכל המספרים כאן **לא** השתנו — היא מכוונת,
  // הערב מבוקש יותר. הסולם המקורי היה `maxSlots <= 5 ? 1 : 2`; כאן הוא רק
  // הורחב לקצה החדש (8).
 ...
 *      ותשמור על הפיזור בין 09:00–13:00 ל-16:00–19:00.
 *
 ...
 * ממנו מחוץ למשמרת (למשל 12:40 להרמת גבות) נופל כאן, כי 13:00 אינו ברשת.
 */
