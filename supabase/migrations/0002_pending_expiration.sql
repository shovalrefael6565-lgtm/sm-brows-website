-- ============================================================================
-- 0002 — תפוגת בקשות pending, הגבלת תפיסת סלוטים, ביטול ע"י הלקוחה
--
-- ⚠️ 0001 כבר רץ בפרויקט Supabase האמיתי. קובץ זה הוא migration חדש בלבד —
-- אין בו שום ALTER/DROP על מה שנוצר שם, רק תוספות.
--
...
-- ערכים חדשים לטיפוסים הקיימים. חייבים לבוא ראשונים בקובץ, לפני כל שימוש
-- בהם — Postgres לא מתיר להשתמש בערך enum חדש באותה פקודה שמוסיפה אותו,
-- אבל מתיר זאת בהמשך אותה טרנזקציה/migration, כל עוד הוא לא נצרך ב-DDL
-- באותו מקום שבו הוא נוצר (כאן הוא נצרך רק בתוך גוף פונקציות, בזמן ריצה).
alter type appointment_status add value 'expired';
alter type history_action     add value 'expired';

-- ─── מועד תפוגה לכל בקשת pending ──────────────────────────────────────────
-- null עבור שורות שנוצרו לפני 0002 (לא יפוגו אף פעם — לא ניגע בעבר).
