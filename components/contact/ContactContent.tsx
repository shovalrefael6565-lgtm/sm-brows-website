...
const hours = [
  { day: 'ראשון – חמישי (בוקר)', time: '09:00 – 12:00' },
  { day: 'ראשון – חמישי (אחה"צ)', time: '16:00 – 19:00' },
...
const contactItems = [
  {
    ...
    value: LOCATION,
    /*
      ⚠️ שאילתת המפה חייבת להתאים למה שהאתר מפרסם, ולא לכתובת המלאה.

      עד כאן היה כאן `?q=הכורמים+אשקלון` — שם הרחוב. הטקסט הגלוי הוא
      LOCATION ("עיר היין, אשקלון") בלבד, אבל ה-href נשלח לגוגל ונשמר
      בהיסטוריית הדפדפן, כלומר הרחוב כן התפרסם בפועל דרך הקישור.
      ההנחיה העסקית היא לפרסם רק את השכונה; הכתובת המדויקת נמסרת
      אישית בוואצאפ עם אישור התור (lib/whatsappTemplates.ts).

      ⚠️ אין להחזיר לכאן רחוב, מספר בית, קומה או דירה.
    */
