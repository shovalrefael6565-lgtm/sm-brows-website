...
export default function CustomerPicker({
  ...
  async function createCustomer(e: React.FormEvent) {
    ...
    try {
      ...
      /*
       * ⚠️ 200 בלי מזהה — שריד ההגנה מהתקלה שבה השרת החזיר הצלחה בלי
       * `customerId` (שם השדה לא תורגם מ-`customer_id`), והבורר הציג
       * "יצירת הלקוחה נכשלה" על לקוחה שכבר נשמרה ב-CRM.
       *
       * השרת אינו יכול להגיע למצב הזה יותר, ולכן זו הודעת אמת ולא
       * "נסי שוב": הלקוחה **כן** עשויה להיות שמורה, וניסיון חוזר רק
       * ייצור בלבול. חפשי אותה בשם או במספר.
       */
      if (!data.customerId) {
        setCreateError('הלקוחה נשמרה אך לא הוחזר מזהה. חפשי אותה בשם או במספר — אין ליצור אותה שוב.')
        return
      }
